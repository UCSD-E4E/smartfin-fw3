 ...
 * empty. No locking; safe only for one producer thread and one consumer thread.
 *
...
private:
    ...
    std::size_t head_;        //!< Producer index (next write position).
    std::size_t tail_;        //!< Consumer index (next read position).
};
