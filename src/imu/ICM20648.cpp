...
#define ICM20648_BIT_INT_ANYRD_2CLEAR    0x10                        /**< Clear on any reg read                                  */
#define ICM20648_BIT_INT_BYPASS_EN       0x02                        /**< I2C bypass enable                                      */
...
ICM20648::ICM20648(uint8_t address): m_accelRawData{0}, m_gyroRawData{0}
{
    m_address = address;
}
#else
...
#define ICM20648_ACCEL_BANDWIDTH    ICM20648_ACCEL_BW_246HZ
#define ICM20648_ACCEL_FULLSCALE    ICM20648_ACCEL_FULLSCALE_2G
#define ICM20648_GYRO_BANDWIDTH     ICM20648_GYRO_BW_200HZ
#define ICM20648_GYRO_FULLSCALE     ICM20648_GYRO_FULLSCALE_250DPS
#endif
...
bool ICM20648::open()
{
    ...
    set_accel_sample_rate(1100);
    set_accel_bandwidth(ICM20648_ACCEL_BANDWIDTH);
    set_accel_fullscale(ICM20648_ACCEL_FULLSCALE);
    set_gyro_sample_rate(1100);
    set_gyro_bandwidth(ICM20648_GYRO_BANDWIDTH);
    set_gyro_fullscale(ICM20648_GYRO_FULLSCALE);

    /* INT pin: active low, push-pull, interrupt latched, reg read clears, bypass enabled */
    write_register(ICM20648_REG_INT_PIN_CFG, ICM20648_BIT_INT_ACTL | ICM20648_BIT_INT_LATCH_EN | ICM20648_BIT_INT_ANYRD_2CLEAR | ICM20648_BIT_INT_BYPASS_EN);
...
#if PARTICLE_IO
    m_I2C.write(m_address, (const char*)&regAddr, sizeof(regAddr), true);
...
{
#if PARTICLE_IO
    uint8_t data[2] = { ICM20648_REG_BANK_SEL, (uint8_t)(bank << 4) };
    m_I2C.write(m_address, (const char*)data, sizeof(data), false);
#else
    /* Enable chip select */
    m_CS = 0;
...
#if PARTICLE_IO
    memcpy(m_accelRawData, rawData, sizeof(rawData));
#endif
...
#if PARTICLE_IO
    memcpy(m_gyroRawData, rawData, sizeof(rawData));
#endif
