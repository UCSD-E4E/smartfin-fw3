...
/**
 * @brief Flag to allow deinit_conio to distinguish if SF_OSAL_getline has already written file_buf
 * into log file
 *
 */
            ...
            if (conioHistory.get_display() && conioHistory.is_active())
            {
                ...
                case KEY_UP:
                {
                    ...
                    if (conioHistory.get_cur_bottom_display() == conioHistory.get_bottom_display())
                    {
                        ...
                        conioHistory.overwrite_last_line_at(
                            file_buf.c_str(), file_buf.size(), offset, false);
                        buf_written = true;
                    }
                    wscrl(stdscr, -1);
                    curs_set(0);
                ...
                case KEY_DOWN:
                {
                    ...
                    char *line =
                        conioHistory.retrieve_display_line(conioHistory.get_cur_bottom_display());
                    if (!line)
                    {
                        break;
                    }
...
extern "C"
{
    ...
    int SF_OSAL_kbhit(void)
    {
        ...
        return rv;
#else
        return SF_HAL::serial_available();
#endif
    }
    ...
    int SF_OSAL_getch(void)
    {
        ...
        return retval;
#else
        while (SF_HAL::serial_available() == 0)
        {
            SF_HAL::delay_ms(1);
        }
    ...
    int SF_OSAL_putch(int ch)
    {
        ...
        return ch;
#else
        SF_HAL::serial_print_char((char)ch);
        return ch;
#endif
    }
    ...
    int SF_OSAL_getline(char *buffer, int buflen)
    {
        ...
        while (i < buflen)
        {
            if (SF_OSAL_kbhit())
            {
                ...
                if (conioHistory.get_cur_bottom_display() != conioHistory.get_bottom_display() &&
                    conioHistory.is_active())
                {
                    wclear(stdscr);
                    char *line;
                    for (std::size_t idx = conioHistory.get_bottom_display() - wind_h + 1;
                         idx < conioHistory.get_bottom_display();
                         idx++)
                    {
                ...
                case '\n':
                    ...
                    SF_OSAL_putch('\n');
                    conioHistory.overwrite_last_line_at(
                        file_buf.c_str(), file_buf.size(), offset, true);
...
#else
        SF_HAL::serial_begin(SF_SERIAL_SPEED);
#endif
    }
