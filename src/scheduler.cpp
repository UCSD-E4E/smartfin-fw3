 ...
 * 
 * This scheduler minimizes delays in tasks with respect to their original start time. 
 * For a task that runs every X ms and takes Y ms to run where X > Y:
 * - If the start of that task is Z ms late, where X > (Y + Z), then it will run at X + Z ms.
 * - The goal of this scheduler is to ensure that the next time this task runs is at 2X ms.
 * - If the delay is greater, such that X < (Y + Z), the scheduler will skip this iteration of the task and run it at the next interval of X from the start time.
 * 
 * The scheduling is done by taking a predetermined maxDuration and using that worst-case scenario to check for overlaps with other tasks. If it can run without overlaps, it will be run; otherwise, the task will be skipped.
 * 
 * In cases where all tasks cause an overlap, the first ensemble in the schedule table will be run. If multiple tasks do not overlap with another task, the earliest ensemble that does not overlap will be run.
 */
